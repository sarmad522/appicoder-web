...
export default function CaseStudyVertical({
  ...
  bgImage = "/images/slideback.png", // optional background behind everything
}) {
  return (
    <section className="relative overflow-hidden  w-[100%] -mt-80  md:-mt-120  -z-10  pt-[50px]  md:pt-[20px]"   >
      {/* Background (blurred image + dark overlay for contrast) */}
      <div className="absolute inset-0 -z-10">
        <Image
          ...
          priority
        />
        <div className="absolute inset-0 bg-black/45   w-[100%]" />
      </div>

      <div className="mx-auto w-[90%] xl:max-w-[1920px] py-10 md:py-14 lg:py-20">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-end">
          {/* LEFT: Text Block */}
          <div className="md:col-span-6 text-white items-end">
            <div className="flex flex-wrap gap-2 mb-4">
              {tags.map((t) => (
                <span
                  key={t}
                  className="rounded-md bg-white text-[#E01923] px-3 py-1 text-xs   font-semibold tracking-tight ring-1 ring-white/10 "
                >
                  {t}
                </span>
              ))}
            ...
            <h2 className="font-bold  text-[24px] md:text-[40px]  lg:text-[48px] leading-tight tracking-tight">
              {title.split(" - ")[0]}{" "}
              <span className="block sm:inline">- {title.split(" - ")[1]}</span>
            </h2>

            <p className="mt-4 max-w-[60ch]   text-[12px] md:text-[14px] lg:text-[16px] text-white text-sm sm:text-base leading-relaxed">
              {description}
            </p>
            ...
            <div className="mt-6">
              <a
                href={ctaHref}
                className="inline-flex items-center justify-center rounded-md bg-gradient-to-b from-[#d01a21] to-[#891117] px-6 py-3 text-white font-semibold shadow-lg ring-1 ring-black/10 hover:translate-y-[1px] transition"
              >
                {ctaText}
              </a>
            </div>
  ...
  centeredSlides
  spaceBetween={50}
  className="h-[720px] md:h-[760px] lg:h-[780px] w-full"
>
  {slides.map((s, i) => (
    <SwiperSlide key={i}>
      <div className="flex items-center justify-center">
        <div className="relative w-full max-w-[650px] md:max-w-[700px] rounded-xl">
          <div className="relative h-[300px] overflow-hidden rounded-lg">
            ...
            {/* Up/Down arrows (fixed at middle-right like screenshot) */}
            <div className="pointer-events-none absolute inset-y-0 right-0 flex flex-col items-center justify-center gap-3 pr-1 md:pr-3">
              <button
                aria-label="Previous"
                className="case-prev pointer-events-auto inline-flex h-10 w-10 items-center justify-center rounded-full bg-white text-black shadow-md hover:scale-105 transition"
              >
                {/* Up chevron */}
...
<path d="M26.4285 14.4077C25.8409 13.8238 24.8911 13.8267 24.3072 14.4143L14.7908 23.9898C14.2069 24.5774 14.2098 25.5271 14.7974 26.1111C15.385 26.6951 16.3348 26.6921 16.9187 26.1045L25.3777 17.593L33.8892 26.0519C34.4768 26.6359 35.4265 26.633 36.0105 26.0454C36.5945 25.4578 36.5915 24.508 36.0039 23.924L26.4285 14.4077ZM25.3711 15.4717L23.8711 15.4763L23.9331 35.4762L25.4331 35.4716L26.9331 35.4669L26.8711 15.467L25.3711 15.4717Z" fill="black"/>
              ...
              <button
                aria-label="Next"
                className="case-next pointer-events-auto inline-flex h-10 w-10 items-center justify-center rounded-full bg-white text-black shadow-md hover:scale-105 transition"
              >
                {/* Down chevron */}
                <svg xmlns="http://www.w3.org/2000/svg" width="51" height="51" viewBox="0 0 51 51" fill="none">
